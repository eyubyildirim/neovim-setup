...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      -- NOTE: mason-lspconfig v2 dropped the `handlers` option. Overrides now go
      -- through `vim.lsp.config`, which merges on top of the defaults that
      -- nvim-lspconfig ships for each server.
  ...
  },
  {
    ...
    config = function()
      ...
      vim.api.nvim_set_keymap('n', '<leader>si', [[<cmd>lua require'telescope'.extensions.goimpl.goimpl{}<CR>]], { noremap = true, silent = true })
    end,
  },
  {
    ...
    config = function()
      ...
      -- Setup autocmd to trigger keymap setup
